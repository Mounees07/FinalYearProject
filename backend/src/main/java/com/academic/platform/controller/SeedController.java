...
public class SeedController {
    ...
    public ResponseEntity<String> seedLmsData(@RequestParam(required = false) String studentUid) {
        ...
        try {
            ...
            for (Course c : courses) {
                // Check if section exists for this specific faculty/sem/year
                // We default to Fall 2026 as per screenshot
                ...
                if (!exists) {
                    Section s = Section.builder()
                            ...
                            .faculty(faculty)
                            .semester("Fall")
                            .year(2026) // Updated to 2026 based on screenshot
                            .build();
                    createdSections.add(sectionRepository.save(s));
                } else {
                    // Section exists, maybe fetch it if we needed it for something else
                    // but for now we just skip creation
                }
            }
            logs.append("Sections verified/created.\n");

            // 4. Create Assignments - SKIPPED BY REQUEST
            // Logic: Create assignments for ALL sections available to catch up if needed
            // (Removed to preventing auto-populating default assignments)
            /*
            ...
            long cseFacultyCount = userRepository.findByDepartmentAndRoleIn("CSE", List.of(Role.TEACHER)).size();
            if (cseFacultyCount < 3) {
                for (int i = 1; i <= 3 - cseFacultyCount; i++) {
                    User t = User.builder()
                            .firebaseUid("seed_fac_uid_" + System.currentTimeMillis() + i)
                            .email("faculty" + System.currentTimeMillis() + i + "@cse.edu")
                            .fullName("CSE Faculty " + i)
                            .role(Role.TEACHER)
                            .department("CSE")
                            .build();
            ...
            long cseStudentCount = userRepository.findByDepartmentAndRoleIn("CSE", List.of(Role.STUDENT)).size();
            if (cseStudentCount < 5) {
                for (int i = 1; i <= 5 - cseStudentCount; i++) {
                    User s = User.builder()
                            .firebaseUid("seed_stu_uid_" + System.currentTimeMillis() + i)
                            .email("student" + System.currentTimeMillis() + i + "@cse.edu")
                            .fullName("CSE Student " + i)
                            .role(Role.STUDENT)
                            .department("CSE")
                            .semester(8)
                            .section("A")
                            .rollNumber("22CSE" + String.format("%03d", i))
                            .build();
            ...
            List<User> cseStudents = userRepository.findByDepartmentAndRoleIn("CSE", List.of(Role.STUDENT));
            ...
            if (hodOpt.isPresent()) {
                User hod = hodOpt.get();
                if (!"CSE".equals(hod.getDepartment())) {
    ...
    public ResponseEntity<String> removeDuplicateSections() {
        try {
            ...
            int removedCount = 0;
            // Simple dedup logic:
            // Group by Course + Faculty + Sem + Year. Keep 1, delete others.
            // Since we can't easily group in memory without hash keys, let's just do a
            // double loop or map.

            // Map key: Set<String> (signature) -> List<Section>
            ...
            for (List<Section> group : groups.values()) {
                if (group.size() > 1) {
                    ...
                    // Better: Keep the one with most enrollments/assignments?
                    // For simplicity, keep index 0, delete 1..n
                    for (int i = 1; i < group.size(); i++) {
                        Section toDelete = group.get(i);
                        // Cascade delete enrollments/assignments for this duplicate section?
                        // Ideally YES.
    ...
    private void createCourseIfNotExists(String code, String name, String desc, Integer credits) {
        if (courseRepository.findByCode(code).isEmpty()) {
            ...
            // Update existing course to have department if missing
