...
import { Search, Plus, SlidersHorizontal, Filter, MoreHorizontal, Phone, Mail, Trash2, Edit, ChevronLeft, ChevronRight, Briefcase, Eye } from 'lucide-react';
...
const AdminTeacherList = () => {
    ...
    const fetchTeachers = async () => {
        setLoading(true);
        try {
            const res = await api.get('/users/faculty'); // Fetch all faculty
    ...
    const toggleSelectTeacher = (id) => {
        ...
        console.log("Selected Faculty:", selectedTeachers); // Debug
    };
    ...
    const openAddModal = () => {
        setModalMode('add');
        setFormData({}); // TeacherDetailsModal handles default structure with Role dropdown
        setShowModal(true);
    };
    ...
    const handleSave = async (data) => {
        try {
            if (modalMode === 'add') {
                const payload = {
                    ...data,
                    // Role is now in data.role from the modal form
                };
    ...
    return (
        <div className="student-list-container">
            ...
            <div className="student-list-header">
                <h2 className="page-title">All Faculty</h2>
                            ...
                            <th className="checkbox-cell">
                                <input
                                    ...
                                    onChange={() => {
                                        ...
                                        else setSelectedTeachers(paginatedTeachers.map(t => t.firebaseUid));
                                    }}
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            paginatedTeachers.map((teacher) => {
                                const isSelected = selectedTeachers.includes(teacher.firebaseUid);
                                return (
                                    <tr
                                        key={teacher.firebaseUid}
                                    ...
                                    >
                                        <td className="checkbox-cell">
                                            <input
                                                type="checkbox"
                                                checked={isSelected}
                                                onChange={() => toggleSelectTeacher(teacher.firebaseUid)}
                                                className="custom-checkbox"
                                            />
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <span className={`role-badge ${teacher.role?.toLowerCase()}`}>
                                                {teacher.role}
                                            </span>
                                        </td>
